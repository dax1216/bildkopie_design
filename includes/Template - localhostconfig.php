...
function siteInfo($name = NULL) {
    ...
    if (isset($name)) {
        if ($name == $template_url) {
            $path = 'http://' . $_SERVER['HTTP_HOST'] . '/' . $path[1] . '/assets/';
            echo $path;
        } elseif ($name == $url) {
...
function tplDir($name = NULL) {
    $path = explode('/', $_SERVER['REQUEST_URI']);
    if (isset($name)) {
        $path = 'http://' . $_SERVER['HTTP_HOST'] . '/' . $path[1] . '/template-files/' . $name;
    } else {
        $path = 'http://' . $_SERVER['HTTP_HOST'] . '/' . $path[1] . '/template-files/';
    }
    return $path;
}
...
function get_header($name = NULL) {
    ...
    if (isset($name)) {
        ...
        $file = $root . '/' . $dir[3] . '/' . $dir[4] . '/' . 'header-' . $name . '.php';

        $dir = explode('/', tplDir());
        if (file_exists($file)) {
            // echo $path = 'http://localhost/real-estate/' . $dir[4] . '/header-'.$name.'.php';
            include $dir[4] . '/header-' . $name . '.php';
        } else {
            include $dir[4] . '/header.php';
        }
        //$dir = tplDir();
        //$dir = explode('/', $dir);
...
function get_template_part($name = NULL) {
    ...
    if (isset($name)) {
        $root = $_SERVER['DOCUMENT_ROOT'];
        $file = $root . '/' . $dir[3] . '/' . $dir[4] . '/' . 'content-' . $name . '.php';
        $dir = explode('/', tplDir());
        if (file_exists($file)) {
            include $dir[4] . '/content-' . $name . '.php';
        } else {
...
function get_footer($name = NULL) {
    ...
    if (isset($name)) {
        $root = $_SERVER['DOCUMENT_ROOT'];
        $file = $root . '/' . $dir[3] . '/' . $dir[4] . '/' . 'footer-' . $name . '.php';
        $dir = explode('/', tplDir());
        if (file_exists($file)) {
            include $dir[4] . '/footer-' . $name . '.php';
        } else {
